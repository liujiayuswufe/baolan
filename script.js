...
function getFundDrawdownValues(unitNet, accumNet) {
  const preferred = Array.isArray(accumNet) ? accumNet : [];
  const hasAccumData = preferred.some((value) => {
  ...
  return Array.isArray(unitNet) ? unitNet : [];
}
...
function calculateWindowMetrics(data, start, end) {
  ...
  const drawdownProfile = calculateDrawdownProfile(getFundDrawdownValues(unitNet, accumNet), dates);
...
function updateCharts() {
  ...
  const fundDrawdown = computeDrawdownSeries(getFundDrawdownValues(unitNet, accumNet));
...
function getSupabaseConfig() {
  ...
  if (/^sb_secret_/i.test(key)) {
    throw new Error("Front-end must use a Supabase publishable key, not a service_role key.");
  }
...
function buildRangeDataFromRows(fundRows, benchmarkRows, benchmarkCode = state.currentBenchmark) {
  ...
  const drawdownValues = getFundDrawdownValues(unitNet, accumNet);
  const fundDrawdown = computeDrawdownSeries(drawdownValues);
  ...
  const fundReturns = computeDailyReturnSeries(unitNet);
  const drawdownProfile = calculateDrawdownProfile(drawdownValues, dates);
...
function sliceMetricWindow(data, days, benchmarkCode = state.currentBenchmark) {
  ...
  return {
    ...
    accumNet: Array.isArray(data?.accum_net) ? data.accum_net.slice(start) : [],
...
function buildRiskIndicatorRowsFromRangeData(data, benchmarkCode = state.currentBenchmark) {
  ...
  return [
    ...
    buildRow("最大回撤", formatPercent, (windowData) => calculateDrawdownProfile(getFundDrawdownValues(windowData.unitNet, windowData.accumNet), windowData.dates).maxDrawdown),
